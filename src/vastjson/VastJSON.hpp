...
public:
   VastJSON()
     : mode{ ModeVastJSON::BIG_ROOT_DICT_GENERIC }
     , ifsptr{ nullptr }
   {
   }
   ...
   VastJSON(std::ifstream&& _if, ModeVastJSON _mode = ModeVastJSON::BIG_ROOT_DICT_GENERIC)
     : mode{ _mode }
     , ifsptr{ new std::ifstream{ std::move(_if) } }
   {
      assert(ifsptr->good());
      if (mode == BIG_STRICT)
         loadStrictFromIfsptr();
